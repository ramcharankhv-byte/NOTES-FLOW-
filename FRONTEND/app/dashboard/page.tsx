...
export default function DashboardPage() {
  ...
  const { workspaces, fetchWorkspaces, createWorkspace, searchWorkspaces } = useWorkspaceStore()
      ...
      >
        ...
        <motion.div variants={itemVariants} className="mb-12">
          <h2 className="text-4xl font-black text-white mb-2">
            Welcome back, <span className="bg-gradient-to-r from-blue-400 to-cyan-300 bg-clip-text text-transparent">{user.username}</span>
          </h2>
          <p className="text-gray-400">Manage your workspaces and collaborate with your team</p>
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="flex flex-col sm:flex-row gap-4 mb-12">
          ...
          <motion.button
            ...
            className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold flex items-center gap-2 transition-all hover:shadow-lg hover:shadow-blue-600/50"
          >
            <Plus size={18} />
            Create Workspace
          </motion.button>
            ...
            {filteredWorkspaces.length > 0 ? (
              filteredWorkspaces.map((workspace) => (
                <motion.div
                  ...
                  className="glass rounded-xl p-6 cursor-pointer group border border-blue-400/20 hover:border-blue-400/60 transition-all"
                >
                  {/* Icon */}
                  <div className="p-4 bg-gradient-to-br from-blue-600/30 to-cyan-600/20 rounded-lg w-fit mb-4 group-hover:from-blue-600/50 group-hover:to-cyan-600/40 transition-all">
                    <Grid3x3 size={24} className="text-blue-400 group-hover:text-cyan-300 transition-colors" />
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-blue-400/10">
                    ...
                    <ArrowRight size={16} className="text-gray-400 group-hover:text-blue-400 transform group-hover:translate-x-1 transition-all" />
                  </div>
                </motion.div>
              ))
            ) : (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                className="col-span-full glass rounded-xl p-12 text-center border border-blue-400/10"
              >
                ...
                <p className="text-gray-400 mb-6">
                  {searchQuery ? 'Try a different search term' : 'Create your first workspace to get started collaborating'}
                </p>
                {!searchQuery && (
                  <motion.button
                    whileHover={{ scale: 1.05 }}
          ...
          <>
            ...
            >
              <div className="glass rounded-2xl p-8 max-w-md w-full border border-blue-400/20">
                ...
                <p className="text-gray-400 mb-6">Give your workspace a name to get started</p>
